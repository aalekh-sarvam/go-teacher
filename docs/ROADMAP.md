...
**Status (2026-09-14, later):** everything below is implemented except C7 (thread tuning, dropped: the user has already benchmarked). Notes on deviations: A2 refutations come from the next position's search, so no extra query; A6 replaced the hand-written joseki database with engine-grounded "opening patterns" (corner point, approach and invasion names derived from the stones, plus KataGo's first deviation per corner), because a typed joseki list risks teaching wrong sequences; A8 runs the target profile as a one-visit pass over every position; C5 is wired through `CODESIGN_IDENTITY` / `NOTARY_PROFILE` environment variables but not exercised (needs a Developer ID); B6's file association is registered in Info.plist and handled via tao's `Event::Opened`.
