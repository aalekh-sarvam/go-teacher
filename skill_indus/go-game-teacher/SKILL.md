...
- **The game arc** (`game_arc`): a short `intro` plus one entry per phase (opening / middle game / endgame) with `phase`, `move_range`, a brief `narrative` (expand only when useful), and optional `turning_points` (short move-tagged notes). This is the holistic commentary — how the strategic balance swung, not a move-by-move list. Follow `references/game-arc-commentary.md` on voice and board-fact discipline.
